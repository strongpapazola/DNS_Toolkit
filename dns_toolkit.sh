...
if [ $confirm = "y" ];then
	echo -e $inp"Copying Default Config To '/etc/bind-default/' ..."
	cp -rf /etc/bind /etc/bind-default
...
read -p "Are You Sure...? [Y/n] " confirm
if [ $confirm = "y" ];then
	echo -e $inp"Restoring Default Config From '/etc/bind-default/' ..."
	cp -rf /etc/bind-default /etc/bind
	echo -e $ins"Added Default Config...!"
	$pause
	banner
	dnsfunction
